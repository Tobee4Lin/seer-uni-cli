		...
		<view :class="{'pickerMask':showPicker}" @click="maskClick" @touchmove.stop.prevent="returnHandle">
			<view class="r-dtpicker" :class="{'r-dtpicker-show':showPicker}">
				...
				<view class="rangeBox" @touchmove.stop.prevent="returnHandle" @tap.stop="returnHandle">
					<input type="text" disabled placeholder="起始日期" :value="startDate" :style="{color:themeColor,'border-color':themeColor,opacity:dateType=='startDate'?1:.5}"
					 @tap="changeDateType('startDate')">至<input type="text" disabled placeholder="截止日期" :style="{color:themeColor,'border-color':themeColor,opacity:dateType=='endDate'?1:.5}"
...
<script>
	export default {
		...
		data() {
			...
			const startDate = `${date.getFullYear() - 1}-01-01`;
			const endDate = `${date.getFullYear() + 1}-12-31`;
			
			let start = parseInt(startDate.slice(0, 4));
			let end = parseInt(endDate.slice(0, 4));
			...
			for (let i = 1; i <= 12; i++) {
				let v = i;
				if (v < 10) v = "0" + v;
				if (startDate.length > 4 && endDate.length > 4 && (startDate.slice(0, 4) == endDate.slice(0, 4))) {
					if (parseInt(v) >= startDate.slice(5, 7) && parseInt(v) <= endDate.slice(5, 7)) {
						monthArr.push(v.toString())
					}
				} else {
					monthArr.push(v.toString())
				}
			...
			return {
				...
				dayArr,
				day,
				pickerValue: [1, month - 1, day - 1],
							
				showPicker: this.show,
				// dayArr: [],
				// pickerValue: this.fields == 'year' ? [0] : this.fields == 'month' ? [0, 0] : [0, 1, 0],
				// pickerValue: this.pickerValues,
				dateType: "startDate",
				startDate: "",
				endDate: ""
			};
		...
		methods: {
			...
			init() {
				...
				var start = this.fields == 'year' ? this.start.slice(0, 4) : this.fields == 'month' ? this.start.slice(0, 7) : this
					.start,
			...
			changeDateType(dateType) {
				...
				if (this[dateType]) {
					...
					var dateTxt = this.fields == 'year' ? this.yearArr[this.pickerValue[0]] : this.fields == 'month' ? this.yearArr[
