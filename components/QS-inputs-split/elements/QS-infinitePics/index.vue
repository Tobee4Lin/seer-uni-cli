		...
		<view class="width100 padding_10rpx_15rpx wrap" :class="itemLayout_computed">
			...
			v-if="max?(itemArray.length>=max?false:true):true">
				...
				@tap="chooseImg">
					...
					<!-- <text class='cuIcon-cameraaddfill text-gray' style="font-size: 40rpx;"></text> -->
					<image src="@/static/picUpload.png" mode="aspectFill" style="width: 40rpx;height: 40rpx;"></image>
				</view>
			</view>
...
<script>
	...
	export default {
		...
		methods: {
			chooseImg() {
				let max = this.max - this.itemArray.length;
				uni.chooseImage({
					// sizeType: ['compressed'],
					count: max,
					success: res => {
