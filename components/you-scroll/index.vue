<template>
	<view class="you-scroll" ref="youScroll">
		<view class="pullDown" :style="{ transform: 'translateY('+translateY+'px)', transition: isDown ? '0s' : '0.3s' }">
			<slot name="pullDown">
				<!-- <img src="../../static/logo.png" :class="{'down-icon': true,'animate': pullDownStatus == 3}"></img> -->
				<span>
	...
	export default {
		...
		methods: {
			...
			endFn(e) {
				...
				setTimeout(() => {
					this.translateY = this.translateY >= this.pullDownDistance ? this.pullDownDistance + 40 : 0;
			...
			scrolltolower(e) {
				...
				view.fields({ 
					...
					console.log(e.detail.scrollHeight - data.height - this.reachBottomDistance)
