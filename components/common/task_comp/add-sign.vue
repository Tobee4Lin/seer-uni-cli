<template>
	<view>
		...
		<view class="wrap">
			<view class="content">
				<view class="top">
					<p class="title">检查方：{{canteenInfo.nickName || ''}}</p>
		...
		<view class="margin-tb-sm lg text-center fix-btn">
			<button class="cu-btn round bg-blue lg shadow" @click="requestAddInspectOrder">确定提交</button>
		</view>
	</view>
</template>
...
<script>
	...
	export default {
		...
		data() {
			return {
				canteenInfo: this.$store.state.canteenInfo,
				picArr: [],
				inspector_sign_id: "",
		...
		methods: {
			...
			async uploadInspectOrder() {
				...
				let data = {
					...
					taskOrderId: this.$store.state.inspectInfo.taskOrderId,
					version: this.$store.state.inspectInfo.version * 1,
					taskBatchVersion: this.$store.state.inspectInfo.taskBatchVersion * 1
				};
				
				API.executeTask(data).then(async res => {
					uni.hideLoading();
