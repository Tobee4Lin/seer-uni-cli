<template>
	<view class="jPicker">
		<view @click="showPicker" class="showLine">
			<view v-if="currentName || nowName" style="font-size: 30rpx;height: 60rpx;line-height: 60rpx;">
		...
		<view class="pickerMask" v-if="pickerVisable" @click.stop="hide">
			<view class="alertArea JAnimateBtmIn" @click.stop="doNothing">
				...
				<view class="">
					<view class="cu-bar search bg-white">
						<view class="action text-gray" @tap="cancelSel">取消</view>
						<view class="search-form round">
							<text class="cuIcon-search"></text>
							<input :adjust-position="false" style="text-align: left;" @input="filterOp" type="text" placeholder="搜索"
							 confirm-type="search"></input>
						</view>
						<view class="action text-blue" @tap="sureSelect">确定</view>
					</view>
				</view>
...
<script>
	...
	export default {
		...
		props: ["options", "nowName", "showKey", "valKey", "val", "position", "disabled", "bgColor", "sureColor"],
		//选项数组，列表显示的对象键名，取值的对象键名,默认选中值,搜索框位置,是否禁用,整体背景色,确认键颜色
		watch: {
			options(n) {
				this.listData = JSON.parse(JSON.stringify(n));
				this.selByValKey();
			},
			val(n) {
				this.selByValKey();
		...
		mounted() {
			this.selByValKey();
			if (this.position) {
				this.searchPosition = JSON.parse(JSON.stringify(this.position));
			}
		},
		methods: {
			showPicker() {
			...
			sureSelect() {
				...
				if (this.listData.length == 0) {
					...
					this.currentName = obj.pickerName;
					if (this.valKey) {
			...
			filterOp(e) {
				...
				if (keyWord != "") {
					...
					this.listData = JSON.parse(JSON.stringify(this.options));
					this.nSel = this.val ? this.val : -1;
					this.selByValKey();
				}
			},
			selByValKey() {
				let n = JSON.parse(JSON.stringify(this.options));
				this.listData = n;
