<template>
	<!-- <view class="HMfilterDropdown" @touchmove.stop.prevent="discard" :style="{'position': 'fixed','z-index': '1000','top': CustomBar +'px'}"> -->
	<view class="HMfilterDropdown" @touchmove.stop.prevent="discard" :style="{'position': 'fixed','z-index': '1000'}">
		<view class="nav">
		...
		<view class="mask" :class="{'show':isShowMask,'hide':maskVisibility!=true}" @tap="togglePage(showPage)" :style="{'top': CustomBar +'px'}"></view>
					...
					<view class="filter">
						<scroll-view class="menu-box" :scroll-y="true">
							<view class="box" v-for="(box,box_index) in page.submenu" :key="box_index">
								...
								<view class="labels">
									<view v-for="(label,label_index) in box.submenu" :key="label_index" @tap="selectFilterLabel(page_index,box_index,label_index,label)"
									 :class="{'on':label.selected}">{{label.name}}</view>
								</view>
							</view>
							<view v-if="inputSearchListCopy.length>0" class="box" v-for="(item, index) in inputSearchListCopy" :key="index">
								<view class="title">{{item.name}}</view>
								<view class="labels" style="margin-top: 10rpx;border: 1rpx solid #C8C7CC;padding: 10rpx 15rpx;border-radius: 10rpx;">
									<input type="text" style="width: 100%;" :placeholder="item.placeholder" v-model="item.input" />
								</view>
							</view>
							<view class="box" v-if="timeSearchTitle">
								<view class="title">{{timeSearchTitle}}</view>
		...
		<rangeDatePick :show="isShow" @showchange="showchange" :value="currentDateArr" @change="bindChange"
			themeColor="#0091FF" fields="day" :pickerValues="pickerValue"></rangeDatePick>
	</view>
</template>
<script>
	import rangeDatePick from '../pyh-rdtpicker/pyh-rdtpicker.vue';
	import {
		generateCurrentDate
	} from "@/utils/date.js";
	export default {
		...
		watch: {
			filterData: {
				handler() {
					...
					this.inputSearchListCopy = this.inputSearchList;
				},
				immediate: true
			},
		...
		methods: {
			generateCurrentDate() {
				...
				this.currentDateArr = [`${date.getFullYear()}-${month}-${day}`];
				
				let monthArr = Array(12).fill(1).map((i, idx) => i = idx + 1);
				let dayArr = Array(31).fill(1).map((i, idx) => i = idx + 1);
				let mIndex = 0;
				let dIndex = 0;
				monthArr.map((i, idx) => {
					if(i == month) {
				...
				this.pickerValue = [1, mIndex, dIndex];
			},
			showPicker(e) {
				this.isShow = true;
			},
			showchange() {
				...
				this.$emit("setTimeSearchSchema");
			},
			bindChange(data) {
				this.selectTimeRange = data;
			...
			initMenu() {
				...
				for (let i = 0; i < this.filterData.length; i++) {
					...
					if (this.filterData.length == 1 && this.isOrder) {
						//仅在采购单模块做处理
						tmpMenu.push({
							name: tmpitem.submenu[1].name,
							type: tmpitem.type
						});
					} else {
						tmpMenu.push({
				...
				this.menu = tmpMenu;
				//初始化选中项数组
				tmpMenuActiveArr = this.defaultSelected.length > 0 ? this.defaultSelected : this.activeMenuArr.length > 0 ? this.activeMenuArr :
					tmpMenuActiveArr;
			...
			selectHierarchyMenu(page_index, level1_index, level2_index, level3_index) {
				this.$emit("changeHierarchyData", {
					index: level1_index,
					name: this.subData[page_index].submenu[level1_index].name
				})
				//读取记录
			...
			resetFilterData(page_index) {
				...
				this.inputSearchListCopy.map(item => {
					item.input = "";
				})
				this.chooseDate = "选择时间段";
				this.$emit("clearTimeSearchSchema");
			...
			selectFilterLabel(page_index, box_index, label_index,label) {
				...
				if(label_index === 0 && label.name === "全部") {
					this.activeMenuArr[page_index][box_index] = [];
					this.subData[page_index].submenu[box_index].submenu.map(i => i.selected = false);
					if (find_index <= -1) {
					...
					this.activeMenuArr[page_index][box_index] = this.activeMenuArr[page_index][box_index].filter(i => i !== 0);
					this.subData[page_index].submenu[box_index].submenu.map(i => {
						if(i.name === "全部") {
							i.selected = false;
						}
					})
			...
			confirm() {
				...
				//对结果做一下处理
				if (!this.is_process_control_report) {
				...
				this.$emit('confirm', {
					index: index,
					value: value,
					inputSearchResult: this.inputSearchListCopy && this.inputSearchListCopy.length > 0 ? this.inputSearchListCopy : [],
					timeSearchRangeResult: this.selectTimeRange
				});
			},
			//show菜单页
			showPageLayer(index) {
